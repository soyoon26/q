...
#### ◾ 🐶 SOSO(박소윤) 
 ...
 프로젝트 첫날 바로 명세서에 나와있는 필수사항부터 개발을 시작했지만 중간점검때 교수님이 필수사항이 필수가 아니고 기획서는 필수사항에 없지만 필요하다고 말씀하셔서 혼란이 왔다. 그때는 이해가 가지 않았지만 그냥 받아들이고 다시 기획서를 작성하고 나니 기능을 구현할 때도 그에 맞춰서 할 수 있었고 어떤 기능을 빼고 넣을지 생각이 정립되어 왜 기획서를 강조하셨는지 깨달았다. 이러한 시행착오가 있었지만 이번 경험으로 인해 다음 프로젝트때는 더 발전된 모습을 보일 수 있을 것이다.
